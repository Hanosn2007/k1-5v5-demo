...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<double>("strategy.contextual_kick.own_half_speed", 0.58);
    declare_parameter<double>("strategy.contextual_kick.opponent_half_speed", 0.75);
    declare_parameter<double>("strategy.contextual_kick.penalty_area_speed", 0.92);
    declare_parameter<double>("strategy.contextual_kick.opponent_half_x", 0.0);
    declare_parameter<double>("strategy.contextual_kick.penalty_area_buffer", 0.0);
    declare_parameter<bool>("strategy.segmented_visual_kick.enable", true);
    declare_parameter<bool>("strategy.segmented_visual_kick.enable_own_half", true);
    declare_parameter<bool>("strategy.segmented_visual_kick.enable_opponent_open_field", false);
    declare_parameter<bool>("strategy.segmented_visual_kick.enable_opponent_penalty_area", true);
    declare_parameter<double>("strategy.segmented_visual_kick.own_half_power", 6.5);
    declare_parameter<double>("strategy.segmented_visual_kick.penalty_area_power", 10.0);
    declare_parameter<double>("strategy.segmented_visual_kick.default_power", 6.0);
    declare_parameter<double>("strategy.segmented_visual_kick.own_half_x_max", 0.0);
    declare_parameter<double>("strategy.segmented_visual_kick.penalty_area_buffer", 0.0);
...
void Brain::pubKickMsg() {
    ...
    bool segmentedVisualKick = true;
    double ownHalfPower = 6.5;
    double penaltyAreaPower = 10.0;
    double defaultPower = 6.0;
    double ownHalfXMax = 0.0;
    double penaltyAreaBuffer = 0.0;
    get_parameter("strategy.segmented_visual_kick.enable", segmentedVisualKick);
    get_parameter("strategy.segmented_visual_kick.own_half_power", ownHalfPower);
    get_parameter("strategy.segmented_visual_kick.penalty_area_power", penaltyAreaPower);
    get_parameter("strategy.segmented_visual_kick.default_power", defaultPower);
    get_parameter("strategy.segmented_visual_kick.own_half_x_max", ownHalfXMax);
    get_parameter("strategy.segmented_visual_kick.penalty_area_buffer", penaltyAreaBuffer);

    const bool ballInOwnHalf = ball_x <= ownHalfXMax;
    const bool ballInOpponentPenaltyArea =
        ball_x > fd.length / 2.0 - fd.penaltyAreaLength - penaltyAreaBuffer &&
        fabs(ball_y) < fd.penaltyAreaWidth / 2.0 + penaltyAreaBuffer;

    double power = defaultPower;
    if (segmentedVisualKick) {
        if (ballInOpponentPenaltyArea) {
            power = penaltyAreaPower;
...
void Brain::updateCostToKick() {
    ...
    const double OBSTACLE_COST_DIST = 1.8;
    const double OBSTACLE_COST = 1.5;
    if (distToObstacle(data->ball.yawToRobot) < OBSTACLE_COST_DIST) {
    ...
    for (int i = 0; i < HL_MAX_NUM_PLAYERS; i++) {
        ...
        const double BUMP_DIST = 1.3;
        const double BUMP_COST = 3.0;
        double lateralGap = fabs(sin(deltaTheta)) * range_tm2ball;
        if (range_tm2ball < range_me2ball && lateralGap < BUMP_DIST) {
            cost += BUMP_COST;
            log_(format("bump cost: %.1f", BUMP_COST));
        }
    }
