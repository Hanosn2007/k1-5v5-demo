...
namespace {
double resolveContextualKickSpeed(Brain *brain, double speedLimit)
{
    double ownHalfSpeed = 0.58;
    double opponentHalfSpeed = 0.75;
    double penaltyAreaSpeed = 0.92;
    double opponentHalfX = 0.0;
    double penaltyAreaBuffer = 0.0;
    brain->get_parameter("strategy.contextual_kick.own_half_speed", ownHalfSpeed);
    brain->get_parameter("strategy.contextual_kick.opponent_half_speed", opponentHalfSpeed);
    brain->get_parameter("strategy.contextual_kick.penalty_area_speed", penaltyAreaSpeed);
    brain->get_parameter("strategy.contextual_kick.opponent_half_x", opponentHalfX);
    brain->get_parameter("strategy.contextual_kick.penalty_area_buffer", penaltyAreaBuffer);

    const auto &fd = brain->config->fieldDimensions;
    const double penaltyLineX = fd.length / 2.0 - fd.penaltyAreaLength - penaltyAreaBuffer;
    const double ballX = brain->data->ball.posToField.x;

    double targetSpeed = ownHalfSpeed;
    if (ballX >= opponentHalfX) {
        targetSpeed = opponentHalfSpeed;
    }
    if (ballX >= penaltyLineX) {
        targetSpeed = penaltyAreaSpeed;
    }

    return min(speedLimit, targetSpeed);
}

bool allowSegmentedVisualKick(Brain *brain)
{
    bool segmentedVisualKick = true;
    bool enableOwnHalf = true;
    bool enableOpponentOpenField = false;
    bool enableOpponentPenaltyArea = true;
    double ownHalfXMax = 0.0;
    double penaltyAreaBuffer = 0.0;
    brain->get_parameter("strategy.segmented_visual_kick.enable", segmentedVisualKick);
    brain->get_parameter("strategy.segmented_visual_kick.enable_own_half", enableOwnHalf);
    brain->get_parameter("strategy.segmented_visual_kick.enable_opponent_open_field", enableOpponentOpenField);
    brain->get_parameter("strategy.segmented_visual_kick.enable_opponent_penalty_area", enableOpponentPenaltyArea);
    brain->get_parameter("strategy.segmented_visual_kick.own_half_x_max", ownHalfXMax);
    brain->get_parameter("strategy.segmented_visual_kick.penalty_area_buffer", penaltyAreaBuffer);

    if (!segmentedVisualKick) return true;

    const auto &fd = brain->config->fieldDimensions;
    const auto &ball = brain->data->ball.posToField;
    const bool ballInOwnHalf = ball.x <= ownHalfXMax;
    const bool ballInOpponentPenaltyArea =
        ball.x > fd.length / 2.0 - fd.penaltyAreaLength - penaltyAreaBuffer &&
        fabs(ball.y) < fd.penaltyAreaWidth / 2.0 + penaltyAreaBuffer;

    if (ballInOpponentPenaltyArea) return enableOpponentPenaltyArea;
    if (ballInOwnHalf) return enableOwnHalf;
    return enableOpponentOpenField;
}
}
...
NodeStatus CamTrackBall::tick()
{
    ...
    const double pixToleranceX = brain->config->camPixX * 1 / 4.; // 球距离视野中心的像素差小于这个容差, 则认为在视野中心了.
    const double pixToleranceY = brain->config->camPixY * 1 / 4.;
    ...
    { // 没看见, 看向记忆中球的大致位置
        ...
        double smoother = 2.6; // 越大头部运动越平滑, 越小则越快, 小于 1.0 会超调震荡
...
NodeStatus StrikerDecide::tick() {
    ...
    if (!(iKnowBallPos || tmBallPosReliable))
    {
        ...
        allowSegmentedVisualKick(brain) &&
        brain->data->tmImLead &&
...
NodeStatus Kick::onStart()
{
    ...
    if (_state == "kick") {
        ...
        double speed = resolveContextualKickSpeed(brain, getInput<double>("speed_limit").value());
        bool softKickoff;
